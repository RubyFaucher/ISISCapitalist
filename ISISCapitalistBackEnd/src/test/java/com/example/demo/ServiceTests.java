...
public class ServiceTests {
    ...
    String serviceUrl = "http://localhost:8080/adventureisis/generic/";
    ...
    private int calcMaxCanBuy(ProductType p, double money) {
        return (int) Math
                .floor(Math.log(1 - (money / p.getCout()) * (1 - p.getCroissance())) / Math.log(p.getCroissance()));
    }
    ...
    private void checkUnlocks(ProductType p) {
        Stream<PallierType> unlocks = p.getPalliers().getPallier().stream()
                .filter(u -> !u.isUnlocked() && u.getSeuil() < p.getQuantite());
        unlocks.forEach(u -> {
            u.setUnlocked(true);
            applyUpgrade(u, p);
        });
    ...
    public void checkXMLValidation() {
        ...
        given().header("Accept", "application/xml").when().get(serviceUrl + "world").then().assertThat()
                .body(matchesXsd(getSystemResourceAsStream("worldSchema.xsd")));
    }

    @Test
    public void checkJSONlogo() {
        System.out.println("Testing : checkJSONlogo");
        // teste si le monde retourné au format JSON contient bien un world.logo qui
        // désigne une image (jpg, gif, ou png)
        given().header("Accept", "application/json").when().get(serviceUrl + "world").then().assertThat().body("logo",
                anyOf(endsWith(".jpg"), endsWith(".gif"), endsWith(".png")));
    }
    ...
    public void checkSixProducts() {
        ...
        given().header("Accept", "application/json").when().get(serviceUrl + "world").then().assertThat()
                .body("products.product", hasSize(6));
    }
    ...
    public void checkThreeUnlocksByProducts() {
        ...
        given().header("Accept", "application/json").when().get(serviceUrl + "world").then().assertThat()
                .body("products.product.findAll { it.palliers.pallier.size()>=3 }", hasSize(6));
    }
    ...
    public void checkAManagerForEachProduct() {
        ...
        int nproduct = given().header("Accept", "application/json").when().get(serviceUrl + "world").then().extract()
                .path("products.product.size()");
        int nmanager = given().header("Accept", "application/json").when().get(serviceUrl + "world").then().extract()
                .path("managers.pallier.size()");
    ...
    public void checkAtLeastOneFirstProduct() {
        ...
        given().header("Accept", "application/json").header("X-User", uniqueID).when().get(serviceUrl + "world").then()
                .assertThat().body("products.product[0].quantite", equalTo(1));
    }

    @Test
    public void checkLaunchProductionFirstProduct() throws InterruptedException {
        System.out.println("Testing : checkLaunchProductionFirstProduct");
        // teste la bonne mise en production du premier produit
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ...
        World w2 = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
    ...
    @Test
    public void checkLaunchAndBuyProduct() throws InterruptedException {
        System.out.println("Testing : checkLaunchAndBuyProduct");
        // teste l'achat du premier produit avec accumulation d'assez d'argent pour
        // l'acheter
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ProductType p = w.getProducts().getProduct().get(0);
        ...
        World w2 = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        // on vérifie qu'on a bien dépensé l'argent pour achat et qu'on a un produit en
        // plus (donc deux)
    ...
    public void checkUnlockFirstProduct() throws InterruptedException {
        ...
        // teste l'achat de plusieurs exemplaires du premier produit avec l'activation
        // de l'unlock et sa bonne prise en compte sur les revenus
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ProductType p = w.getProducts().getProduct().get(0);
        ...
        // on teste la prochaine production pour voir si le serveur calcule toujours
        // bien
        given().contentType("application/json").header("X-User", uniqueID).body(p).put(serviceUrl + "product");
        // on attend un peu plus que le temps de production
        Thread.sleep(p.getVitesse() + 100);
        // on recharge le monde
        World w3 = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        // on vérifie qu'on a bien gagné l'argent attendu à 0.1 de précision pret
    ...
    public void checkBuyManager() throws InterruptedException {
        ...
        // teste l'achat de premier manager. Cela suppose d'accumuler assez d'argent
        // pour y parvenir.
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ProductType p = w.getProducts().getProduct().get(0);
        // on cherche le manager du premier produit et on vérifie qu'on le trouve
        Optional<PallierType> manager = w.getManagers().getPallier().stream().filter(m -> m.getIdcible() == p.getId())
                .findFirst();
        ...
        given().contentType("application/json").header("X-User", uniqueID).body(manager.get())
                .put(serviceUrl + "manager");
        // a partir de maintenant la production est automatisée. On mémorise le
        // timestamp.
    ...
    public void checkUpgradeFirstProduct() throws InterruptedException {
        ...
        // teste l'achat du premier upgrade du premier produit et sa bonne prise en
        // compte sur les revenus
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ProductType p = w.getProducts().getProduct().get(0);
        // on cherche le premier upgrade du produit et on vérifie qu'il existe
        Optional<PallierType> upgrade = w.getUpgrades().getPallier().stream().filter(m -> m.getIdcible() == p.getId())
                .findFirst();
        ...
        given().contentType("application/json").header("X-User", uniqueID).body(upgrade.get())
                .put(serviceUrl + "upgrade");
    ...
    public void checkDeleteWorld() throws InterruptedException {
        ...
        // teste la bonne remise à zéro du monde avec conservation du score sur un
        // delete
        World w = given().header("Accept", "application/json").header("X-User", uniqueID).get(serviceUrl + "world")
                .as(World.class);
        ProductType p = w.getProducts().getProduct().get(0);
